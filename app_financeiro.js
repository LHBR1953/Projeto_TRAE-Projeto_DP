...
async function fetchTransactions(patientId = null) {
    try {
        ...
        const empresaId = (window.currentEmpresaId || (typeof currentEmpresaId !== 'undefined' ? currentEmpresaId : '') || 'emp_8126ebf563');
        ...
        if (!data || data.length === 0) {
            ...
            showToast(`Nenhum lançamento encontrado para a unidade [${empresaId || '-'}].`, true);
            return;
        }
...
async function deleteTransaction(id) {
    try {
        ...
        if (trans.categoria === 'PAGAMENTO' && trans.referencia_id) {
            ...
            const refRaw = trans.referencia_id;
            if (refRaw != null && !isNaN(refRaw)) {
                targetSeqId = Number(refRaw);
            } else {
                const bEncontrado = (budgets || []).find(b => String(b.id || '') === String(refRaw) || String(b.seqid || '') === String(refRaw));
                if (bEncontrado && bEncontrado.seqid != null && !isNaN(bEncontrado.seqid)) targetSeqId = Number(bEncontrado.seqid);
            }

            // orcamento_pagamentos.orcamento_id aceita apenas o seqid numerico do orcamento
            if (targetSeqId != null) {
                const { error: pError } = await db.from('orcamento_pagamentos')
                    .delete()
                    .eq('orcamento_id', targetSeqId)
                ...
                if (!paysRes.error) {
                    ...
                    budgets.forEach(b => {
                        ...
                        const bPayments = allPayments.filter(p => {
                            const idBanco = String(p && p.orcamento_id || '').trim();
...
// --- Movimentação Diária ---
window.loadMovimentacaoDiaria = function loadMovimentacaoDiaria() {
    ...
    if (movDiariaProfessional && window.professionals) {
    ...
    if (!movDiariaModal.dataset.bound) {
        ...
        const closeModal = () => movDiariaModal.classList.add('hidden');
        if (btnCloseMovDiariaModal) btnCloseMovDiariaModal.addEventListener('click', closeModal);
        if (btnCancelMovDiaria) btnCancelMovDiaria.addEventListener('click', closeModal);
        ...
        movDiariaModal.addEventListener('click', (e) => { if (e.target === movDiariaModal) closeModal(); });
...
async function printMovimentacaoDiaria({ dateStr, profSeqId }) {
    ...
    const renderMovDiariaPrint = ({ rows, empId }) => {
        ...
        const profName = profSeqId && typeof getProfessionalNameBySeqId !== 'undefined' ? getProfessionalNameBySeqId(profSeqId) : 'Todos';
...
<body>
  <div class="header">
    ...
    <div class="sub">Unidade: ${escapeHtml(String(empId || '—'))}</div>
  </div>
  ...
  <div class="footer">Documento interno • Movimentação Diária</div>
        ...
        if (typeof openOCCReportPrintWindowFromLegacyHtml === 'function') {
            openOCCReportPrintWindowFromLegacyHtml({ reportName: 'Movimentação Diária', legacyHtml: html, width: 980, height: 720 });
        } else {
    ...
    const currentEmpresaId = window.currentEmpresaId || 'emp_8126ebf563';
    if (!currentEmpresaId) { showToast('Empresa não identificada.', true); return; }

    const inicioDia = `${dateStr}T00:00:00.000Z`;
    const fimDia = `${dateStr}T23:59:59.999Z`;
    const camposSelect = 'id,orcamento_id,valor_pago,' +
                         'data_pagamento,forma_pagamento,empresa_id';

    const { data: pagamentos, error: errPag } = await db
        .from('orcamento_pagamentos')
        .select(camposSelect)
        .eq('empresa_id', currentEmpresaId)
        .gte('data_pagamento', inicioDia)
        .lte('data_pagamento', fimDia);
    if (errPag) {
        showToast('Falha ao buscar pagamentos do dia.', true);
        return;
    }

    const orcIds = pagamentos ? Array.from(new Set(pagamentos.map(p => p.orcamento_id).filter(Boolean))) : [];
    console.log('[DEBUG OCC] Fazendo bypass e buscando orçamentos para os IDs:', orcIds);

    const { data: orcamentos, error: errOrc } = await db.from('orcamentos').select('id,pacientenome,profissional_id').eq('empresa_id', currentEmpresaId).in('id', orcIds);
    if (errOrc) {
        showToast('Falha ao buscar orçamentos do dia.', true);
        return;
    }

    const orcById = new Map((orcamentos || []).map(o => [String(o.id), o]));
    const professionalsSource = window.professionals || (typeof professionals !== 'undefined' ? professionals : []);
    const getProfName = (profId) => {
        const pid = String(profId ?? '').trim();
        if (!pid) return '—';
        const prof = professionalsSource.find(p => String((p && (p.id ?? p.seqid ?? p.profissional_id ?? p.profissionalId)) ?? '').trim() === pid);
        return prof ? String(prof.nome || '') : pid;
    };

    const rows = (pagamentos || [])
        .map(p => {
            const orc = orcById.get(String(p.orcamento_id || ''));
            const profId = orc ? orc.profissional_id : null;
            const dt = p.data_pagamento ? new Date(p.data_pagamento) : null;
            const hora = dt && !Number.isNaN(dt.getTime()) && typeof formatTimeHHMM === 'function' ? formatTimeHHMM(dt) : '—';
            return {
                hora,
                orcSeq: String(p.orcamento_id || '—'),
                paciente: orc ? String(orc.pacientenome || '—') : '—',
                profissional: getProfName(profId),
                forma: String(p.forma_pagamento || '—'),
                item: `Orçamento #${String(p.orcamento_id || '').trim() || '—'}`,
                valor: Number(p.valor_pago || 0),
                _profId: profId
            };
        })
        .filter(r => !profSeqId || String(r._profId ?? '').trim() === String(profSeqId).trim())
        .map(({ _profId, ...rest }) => rest);

    renderMovDiariaPrint({ rows, empId: currentEmpresaId });
}

const btnMovimentacaoDiaria = document.getElementById('btnMovDiaria');
if (btnMovimentacaoDiaria) {
    btnMovimentacaoDiaria.addEventListener('click', () => {
        if (window.loadMovimentacaoDiaria) window.loadMovimentacaoDiaria();
        else if (typeof loadMovimentacaoDiaria !== 'undefined') loadMovimentacaoDiaria();
    });
}

function setFinanceiroMenuActive(key) {
    const contaBtn = document.getElementById('btnMenuContaPaciente');
    const extratoBtn = document.getElementById('btnMenuExtratoTransacoes');
    const isConta = String(key || '') === 'conta';
    const isExtrato = String(key || '') === 'extrato';
    if (contaBtn) {
        contaBtn.classList.toggle('btn-primary', isConta);
        contaBtn.classList.toggle('btn-secondary', !isConta);
        contaBtn.setAttribute('aria-pressed', isConta ? 'true' : 'false');
    }
    if (extratoBtn) {
        extratoBtn.classList.toggle('btn-primary', isExtrato);
        extratoBtn.classList.toggle('btn-secondary', !isExtrato);
        extratoBtn.setAttribute('aria-pressed', isExtrato ? 'true' : 'false');
    }
}

let financeiroWorkspaceMode = 'conta';

function getFinanceiroEmpresaId() {
    return window.currentEmpresaId || (typeof currentEmpresaId !== 'undefined' ? currentEmpresaId : '') || 'emp_8126ebf563';
}

function getFinanceiroUiRefs() {
    const cardBusca = document.getElementById('finPacienteSearch')?.closest('.card') || null;
    const cardExtrato = document.getElementById('finExtratoCard');
    const cardExtratoTitulo = cardExtrato ? cardExtrato.querySelector('h3') : null;
    const extratoFilterBar = document.getElementById('btnFiltrarExtrato')?.closest('div[style*="border-bottom"]') || null;
    ...
    return { cardBusca, cardExtrato, cardExtratoTitulo, extratoFilterBar, finPainelSaldo, homePanel, reportPanel, finTransacoesBody };
}

function setFinanceiroWorkspaceMode(mode) {
    financeiroWorkspaceMode = String(mode || 'conta');
    const { cardBusca, cardExtrato, cardExtratoTitulo, extratoFilterBar, finPainelSaldo, homePanel, reportPanel } = getFinanceiroUiRefs();
    if (homePanel) homePanel.classList.add('hidden');
    if (reportPanel) reportPanel.classList.add('hidden');
    if (cardExtrato) cardExtrato.style.display = 'block';
    if (financeiroWorkspaceMode === 'conta') {
        if (cardBusca) cardBusca.style.display = 'block';
        if (extratoFilterBar) extratoFilterBar.style.display = 'none';
        if (finPainelSaldo) finPainelSaldo.classList.add('hidden');
        if (cardExtratoTitulo) cardExtratoTitulo.textContent = 'Conta do Paciente';
        setFinanceiroMenuActive('conta');
    } else {
        if (cardBusca) cardBusca.style.display = 'none';
        if (extratoFilterBar) extratoFilterBar.style.display = 'flex';
        if (finPainelSaldo) finPainelSaldo.classList.add('hidden');
        if (cardExtratoTitulo) cardExtratoTitulo.textContent = 'Extrato de Transações';
        setFinanceiroMenuActive('extrato');
    }
}

function renderFinanceiroBodyMessage(message, colspan = 10) {
    const { finTransacoesBody } = getFinanceiroUiRefs();
    if (!finTransacoesBody) return;
    finTransacoesBody.innerHTML = `<tr><td colspan="${colspan}" style="text-align:center; padding: 2rem; color: var(--text-muted);">${escapeHtml(String(message || ''))}</td></tr>`;
}

function formatFinanceiroDateTime(value) {
    const dt = value ? new Date(value) : null;
    return dt && !Number.isNaN(dt.getTime()) ? dt.toLocaleString('pt-BR') : '—';
}

function getPatientMatchList(term) {
    const list = Array.isArray(window.patients) ? window.patients : (typeof patients !== 'undefined' ? patients : []);
    const normalized = String(term || '').trim().toLowerCase();
    const matches = list.filter(p => String(p && p.nome || '').toLowerCase().includes(normalized));
    return matches.sort((a, b) => String(a && a.nome || '').localeCompare(String(b && b.nome || ''), 'pt-BR'));
}

function renderContaPacienteSelector(list, titleText) {
    const { finTransacoesBody } = getFinanceiroUiRefs();
    if (!finTransacoesBody) return;
    const rows = (Array.isArray(list) ? list : []).slice(0, 80);
    if (!rows.length) {
        renderFinanceiroBodyMessage('Nenhum paciente encontrado.');
        return;
    }
    finTransacoesBody.innerHTML = rows.map((p, idx) => `
        <tr>
            <td>${idx + 1}</td>
            <td>—</td>
            <td><strong>${escapeHtml(String(p && p.nome || '—'))}</strong></td>
            <td>—</td>
            <td>Paciente</td>
            <td>Conta</td>
            <td style="text-align:right;">—</td>
            <td style="text-align:center;">SELECAO</td>
            <td>${escapeHtml(titleText || 'Selecione um paciente para abrir a conta.')}</td>
            <td style="text-align:center;"><button class="btn btn-secondary btn-sm js-open-conta-paciente" data-patient-id="${escapeHtml(String(p && p.id || ''))}">Abrir Conta</button></td>
        </tr>
    `).join('');
    finTransacoesBody.querySelectorAll('.js-open-conta-paciente').forEach(btn => {
        btn.addEventListener('click', () => {
            const patientId = String(btn.getAttribute('data-patient-id') || '').trim();
            const patient = rows.find(p => String(p && p.id || '') === patientId);
            if (patient) void renderContaPaciente(patient);
        });
    });
}

async function renderContaPaciente(patient) {
    const patientId = String(patient && patient.id || '').trim();
    const patientSeqId = String(patient && patient.seqid || '').trim();
    const patientName = String(patient && patient.nome || 'Paciente').trim();
    const empresaId = getFinanceiroEmpresaId();
    const { finTransacoesBody, finPainelSaldo } = getFinanceiroUiRefs();
    if (!finTransacoesBody) return;
    finTransacoesBody.innerHTML = '<tr><td colspan="10" style="text-align:center; padding: 2rem; color: var(--text-muted);">Carregando conta do paciente...</td></tr>';

    try {
        const budgetQueries = [];
        // Filtra pelo nome do paciente (campo de texto seguro) para evitar erro de bigint
        if (patientName && patientName !== 'Paciente' && patientName !== '—') {
            budgetQueries.push(db.from('orcamentos').select('id,seqid,pacienteid,pacientenome,status,tipo').eq('empresa_id', empresaId).eq('pacientenome', patientName));
        }
        // Se houver ID numérico válido, busca por ele também
        if (patientSeqId && !isNaN(patientSeqId)) {
            budgetQueries.push(db.from('orcamentos').select('id,seqid,pacienteid,pacientenome,status,tipo').eq('empresa_id', empresaId).eq('pacienteid', Number(patientSeqId)));
        }
        const budgetResults = await Promise.all(budgetQueries.map(q => q));
        const budgetMap = new Map();
        budgetResults.forEach(({ data, error }) => {
            if (error) throw error;
            (data || []).forEach(item => {
                const key = String(item && item.id || item && item.seqid || '');
                if (key) budgetMap.set(key, item);
            });
        });

        // Coleta estritamente os números e garante que NENHUMA string de texto passe
        const budgetIds = Array.from(budgetMap.values())
            .map(item => {
                const val = item && item.seqid;
                return (val && !isNaN(val)) ? Number(val) : null;
            })
            .filter(val => val !== null && typeof val === 'number');

        console.log('[DEBUG OCC] IDs numéricos blindados para a query:', budgetIds);

        const paymentTotalByBudgetId = new Map();
        if (budgetIds.length > 0) {
            const { data: pagamentos, error: pagamentosError } = await db
                .from('orcamento_pagamentos')
                .select('id,orcamento_id,valor_pago')
                .in('orcamento_id', budgetIds);
            if (pagamentosError) throw pagamentosError;
            (pagamentos || []).forEach((pag) => {
                const budgetId = String(pag && pag.orcamento_id || '');
                if (!budgetId) return;
                const prev = Number(paymentTotalByBudgetId.get(budgetId) || 0);
                paymentTotalByBudgetId.set(budgetId, prev + Number(pag.valor_pago || 0));
            });
        }

        const debitQueries = [];
        if (patientSeqId && !isNaN(patientSeqId)) debitQueries.push(db.from('financeiro_transacoes').select('id,paciente_id,categoria,tipo,valor,data_transacao,forma_pagamento,observacoes').eq('empresa_id', empresaId).eq('paciente_id', Number(patientSeqId)));
        // REMOVIDA: A busca textual em paciente_id (que é bigint) foi removida para eliminar o erro 400
        const debitResults = await Promise.all(debitQueries.map(q => q));
        const debitMap = new Map();
        debitResults.forEach(({ data, error }) => {
            if (error) throw error;
            (data || []).forEach(item => {
                const key = String(item && item.id || '');
                if (key) debitMap.set(key, item);
            });
        });

        const budgetRows = Array.from(budgetMap.values()).map((orc) => ({
            ref: orc && orc.seqid != null ? `#${orc.seqid}` : '—',
            paciente: String(orc && orc.pacientenome || patientName || '—'),
            data: null,
            categoria: String(orc && orc.tipo || 'ORÇAMENTO'),
            forma: String(orc && orc.status || '—'),
            valor: Number(paymentTotalByBudgetId.get(String(orc && orc.seqid || '')) || 0),
            tipo: 'ORÇAMENTO',
            observacao: `Status: ${String(orc && orc.status || '—')} • Total pago calculado em pagamentos`,
            acao: '—'
        }));

        const debitRows = Array.from(debitMap.values())
            .filter((tx) => String(tx && tx.tipo || '').toUpperCase() === 'DEBITO')
            .map((tx) => ({
                ref: '—',
                paciente: patientName || '—',
                data: tx && tx.data_transacao,
                categoria: String(tx && tx.categoria || 'DÉBITO'),
                forma: String(tx && tx.forma_pagamento || '—'),
                valor: Number(tx && tx.valor || 0),
                tipo: 'DEBITO',
                observacao: String(tx && tx.observacoes || '—'),
                acao: '—'
            }));

        const rows = [...budgetRows, ...debitRows].sort((a, b) => new Date(b.data || 0).getTime() - new Date(a.data || 0).getTime());
        if (!rows.length) {
            renderFinanceiroBodyMessage(`Nenhum orçamento ou débito encontrado para ${patientName}.`);
        } else {
            finTransacoesBody.innerHTML = rows.map((row, idx) => `
                <tr>
                    <td>${idx + 1}</td>
                    <td>${escapeHtml(String(row.ref || '—'))}</td>
                    <td><strong>${escapeHtml(String(row.paciente || '—'))}</strong></td>
                    <td>${escapeHtml(formatFinanceiroDateTime(row.data))}</td>
                    <td>${escapeHtml(String(row.categoria || '—'))}</td>
                    <td>${escapeHtml(String(row.forma || '—'))}</td>
                    <td style="text-align:right;"><strong>${escapeHtml(formatCurrencyBRL(Number(row.valor || 0)))}</strong></td>
                    <td style="text-align:center;">${escapeHtml(String(row.tipo || '—'))}</td>
                    <td>${escapeHtml(String(row.observacao || '—'))}</td>
                    <td style="text-align:center;">${escapeHtml(String(row.acao || '—'))}</td>
                </tr>
            `).join('');
        }
        ...
        if (typeof updateBalanceUI === 'function') {
            await updateBalanceUI(patientSeqId || patientId);
        }
    } catch (error) {
        console.error('Erro ao carregar conta do paciente:', error);
        showToast(`Falha ao carregar a conta do paciente: ${error.message || 'Erro desconhecido'}`, true);
        renderFinanceiroBodyMessage('Erro ao carregar conta do paciente.');
    }
}

async function handleContaPacienteSearch() {
    const input = document.getElementById('finPacienteSearch');
    const term = String(input && input.value || '').trim();
    if (!term) {
        showToast('Digite o nome de um paciente para buscar.', true);
        return;
    }
    const matches = getPatientMatchList(term);
    if (!matches.length) {
        renderFinanceiroBodyMessage('Nenhum paciente encontrado para a busca.');
        return;
    }
    if (matches.length === 1) {
        await renderContaPaciente(matches[0]);
        return;
    }
    renderContaPacienteSelector(matches, 'Foram encontrados vários pacientes. Escolha um para abrir a conta.');
}

async function fetchExtratoTransacoes() {
    const empresaId = getFinanceiroEmpresaId();
    const start = String((document.getElementById('extratoFilterStart') || {}).value || '').trim();
    const end = String((document.getElementById('extratoFilterEnd') || {}).value || '').trim();
    const { finTransacoesBody } = getFinanceiroUiRefs();
    if (!finTransacoesBody) return;
    finTransacoesBody.innerHTML = '<tr><td colspan="10" style="text-align:center; padding: 2rem; color: var(--text-muted);">Carregando extrato de pagamentos...</td></tr>';

    try {
        let query = db.from('orcamento_pagamentos').select('id,orcamento_id,valor_pago,data_pagamento,forma_pagamento,empresa_id').eq('empresa_id', empresaId).order('data_pagamento', { ascending: false });
        if (start) query = query.gte('data_pagamento', `${start}T00:00:00.000Z`);
        if (end) query = query.lte('data_pagamento', `${end}T23:59:59.999Z`);
        const { data: pagamentos, error: errPag } = await query;
        if (errPag) throw errPag;
        const rows = (pagamentos || []).map((pag) => ({
            data: pag && pag.data_pagamento,
            orcamento: String(pag && pag.orcamento_id || '—'),
            forma: String(pag && pag.forma_pagamento || '—'),
            valor: Number(pag && pag.valor_pago || 0)
        }));

        if (!rows.length) {
            renderFinanceiroBodyMessage('Nenhum pagamento encontrado para o período selecionado.');
            return;
        }
        ...
        finTransacoesBody.innerHTML = rows.map((row, idx) => `
            <tr>
                <td>${idx + 1}</td>
                <td>${escapeHtml(`#${String(row.orcamento || '—')}`)}</td>
                <td>—</td>
                <td>${escapeHtml(formatFinanceiroDateTime(row.data))}</td>
        ...
        console.error('Erro ao carregar extrato de transações:', error);
        showToast(`Falha ao carregar extrato: ${error.message || 'Erro desconhecido'}`, true);
        renderFinanceiroBodyMessage('Erro ao carregar extrato de transações.');
    }
}

window.loadContaPaciente = async function loadContaPaciente() {
    setFinanceiroWorkspaceMode('conta');
    renderFinanceiroBodyMessage('Busque um paciente ou clique em "Ver Todos" para selecionar a conta.');
    const input = document.getElementById('finPacienteSearch');
    if (input) input.focus();
};

window.loadExtratoTransacoes = async function loadExtratoTransacoes() {
    setFinanceiroWorkspaceMode('extrato');
    const startInput = document.getElementById('extratoFilterStart');
    const endInput = document.getElementById('extratoFilterEnd');
    ...
    await fetchExtratoTransacoes();
};

const btnMenuContaPaciente = document.getElementById('btnMenuContaPaciente');
if (btnMenuContaPaciente && !btnMenuContaPaciente.dataset.bound) {
    btnMenuContaPaciente.dataset.bound = '1';
    btnMenuContaPaciente.addEventListener('click', () => {
        if (window.loadContaPaciente) void window.loadContaPaciente();
    });
}

const btnMenuExtratoTransacoes = document.getElementById('btnMenuExtratoTransacoes');
if (btnMenuExtratoTransacoes && !btnMenuExtratoTransacoes.dataset.bound) {
    btnMenuExtratoTransacoes.dataset.bound = '1';
    btnMenuExtratoTransacoes.addEventListener('click', () => {
        if (window.loadExtratoTransacoes) void window.loadExtratoTransacoes();
    });
}

const btnFinBuscarFinanceiro = document.getElementById('btnFinBuscar');
if (btnFinBuscarFinanceiro && !btnFinBuscarFinanceiro.dataset.bound) {
    btnFinBuscarFinanceiro.dataset.bound = '1';
    btnFinBuscarFinanceiro.addEventListener('click', () => {
        if (financeiroWorkspaceMode === 'conta') void handleContaPacienteSearch();
    });
}

const btnFinVerTodosFinanceiro = document.getElementById('btnFinVerTodos');
if (btnFinVerTodosFinanceiro && !btnFinVerTodosFinanceiro.dataset.bound) {
    btnFinVerTodosFinanceiro.dataset.bound = '1';
    btnFinVerTodosFinanceiro.addEventListener('click', () => {
        if (financeiroWorkspaceMode === 'conta') {
            const list = getPatientMatchList('');
            renderContaPacienteSelector(list, 'Lista geral de pacientes para abertura da conta.');
        }
    });
}

const finPacienteSearchFinanceiro = document.getElementById('finPacienteSearch');
if (finPacienteSearchFinanceiro && !finPacienteSearchFinanceiro.dataset.bound) {
    finPacienteSearchFinanceiro.dataset.bound = '1';
    finPacienteSearchFinanceiro.addEventListener('keydown', (e) => {
        if (e.key === 'Enter' && financeiroWorkspaceMode === 'conta') {
            e.preventDefault();
            void handleContaPacienteSearch();
        }
    });
}

const btnFiltrarExtratoFinanceiro = document.getElementById('btnFiltrarExtrato');
if (btnFiltrarExtratoFinanceiro && !btnFiltrarExtratoFinanceiro.dataset.bound) {
    btnFiltrarExtratoFinanceiro.dataset.bound = '1';
    btnFiltrarExtratoFinanceiro.addEventListener('click', () => {
        if (financeiroWorkspaceMode === 'extrato') void fetchExtratoTransacoes();
    });
}

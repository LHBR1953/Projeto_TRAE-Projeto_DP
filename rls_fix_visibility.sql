...
-- Compatibilidade: se a tabela foi criada com "user_id", renomeia para "usuario_id"
DO $$
BEGIN
  IF EXISTS (
    SELECT 1
...
-- Helper para evitar recursão (42P17) em policies de usuario_empresas
...
USING (
  EXISTS (
    ...
    WHERE ue.usuario_id = auth.uid()
      AND ue.empresa_id = financeiro_transacoes.empresa_id
  )
  OR COALESCE(auth.jwt() ->> 'email', '') = 'lhbr@lhbr.com.br'
);
...
USING (
  EXISTS (
    ...
    FROM public.usuario_empresas ue
    WHERE ue.usuario_id = auth.uid()
      AND ue.empresa_id = orcamento_cancelados.empresa_id
      AND ue.perfil = 'admin'
  )
  OR COALESCE(auth.jwt() ->> 'email', '') = 'lhbr@lhbr.com.br'
);

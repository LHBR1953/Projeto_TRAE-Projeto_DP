...
begin
  ...
  v_desc_atendimento := nullif(trim(coalesce(to_jsonb(new)->>'descricao_atendimento', '')), '');
  ...
  if v_desc_atendimento is not null then
    v_desc := v_desc || '<p><strong>Evolução:</strong> ' || v_desc_atendimento || '</p>';
  else

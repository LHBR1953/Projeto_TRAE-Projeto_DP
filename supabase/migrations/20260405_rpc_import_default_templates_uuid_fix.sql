...
begin
  ...
  execute format('insert into tmp_inv_map(old_id,new_id) select id::text, gen_random_uuid() from public.%I', v_tpl_inv);
  execute format('insert into tmp_model_map(old_id,new_id) select id::text, gen_random_uuid() from public.%I', v_tpl_models);
  ...
  v_sql := 'insert into public.inventory(id'
        ...
        || (case when coalesce(v_sel,'') <> '' then ', ' || v_sel else '' end)
        || ' '
        || format('from public.%I t join tmp_inv_map m on m.old_id = t.id::text', v_tpl_inv);
  execute v_sql using v_emp;
  ...
  v_sql := 'insert into public.usage_models(id'
        ...
        || (case when coalesce(v_sel,'') <> '' then ', ' || v_sel else '' end)
        || ' '
        || format('from public.%I t join tmp_model_map m on m.old_id = t.id::text', v_tpl_models);
  execute v_sql using v_emp;
  ...
  v_sql := 'insert into public.model_items(model_id, inventory_id'
        ...
        || (case when coalesce(v_sel,'') <> '' then ', ' || v_sel else '' end)
        || ' '
        || format('from public.%I t join tmp_model_map mm on mm.old_id = t.model_id::text join tmp_inv_map im on im.old_id = t.inventory_id::text', v_tpl_model_items);
  execute v_sql using v_emp;
  ...
  v_sql := 'insert into public.service_mapping(service_id, model_id'
        ...
        || (case when coalesce(v_sel,'') <> '' then ', ' || v_sel else '' end)
        || ' '
        || format('from public.%I t join tmp_serv_map sm on sm.old_id = t.service_id::text join tmp_model_map mm on mm.old_id = t.model_id::text', v_tpl_map);
  execute v_sql using v_emp;

...
function liberarFila() { 
    ...
    todosBotoes.forEach(btn => { 
        ...
        if (!btn.classList.contains('finalizado')) { 
            ...
            if (typeof btn !== 'undefined' && btn) { btn.style.opacity = "1"; } 
            if (typeof btn !== 'undefined' && btn) { btn.style.cursor = "pointer"; }
        } 
    }); 
}
...
function renderAtendimentoPlaceholder(msg = 'Selecione a data e o profissional.') {
    if (atendimentoBody) {
        if (typeof atendimentoBody !== 'undefined' && atendimentoBody) { atendimentoBody.innerHTML = `<tr><td colspan="6" style="text-align:center; padding: 2rem; color: var(--text-muted);">${msg}</td></tr>`; }
    }
...
function updateAtendimentoFinalizeButton() {
    ...
    if (n > 0) {
        if (typeof btnAtendimentoFinalizeSelected !== 'undefined' && btnAtendimentoFinalizeSelected) { btnAtendimentoFinalizeSelected.disabled = false; }
        if (typeof btnAtendimentoFinalizeSelected !== 'undefined' && btnAtendimentoFinalizeSelected) { btnAtendimentoFinalizeSelected.innerHTML = `<i class="ri-check-double-line"></i> Finalizar (${n})`; }
    } else {
        if (typeof btnAtendimentoFinalizeSelected !== 'undefined' && btnAtendimentoFinalizeSelected) { btnAtendimentoFinalizeSelected.disabled = true; }
        if (typeof btnAtendimentoFinalizeSelected !== 'undefined' && btnAtendimentoFinalizeSelected) { btnAtendimentoFinalizeSelected.innerHTML = `<i class="ri-check-double-line"></i> Finalizar Selecionados`; }
    }
}
...
function bindAtendimentoFinalizeSingle() {
    ...
    if (typeof atendimentoBody !== 'undefined' && atendimentoBody) { atendimentoBody.addEventListener('click', async (ev) => {
        ...
        if (typeof target !== 'undefined' && target) { target.disabled = true; }
        const originalHtml = target.innerHTML;
        if (typeof target !== 'undefined' && target) { target.innerHTML = '<i class="ri-loader-4-line ri-spin"></i> Processando...'; }
        ...
        try {
            ...
            if (document.body.contains(target)) {
                if (typeof target !== 'undefined' && target) { target.disabled = false; }
                if (typeof target !== 'undefined' && target) { target.innerHTML = originalHtml; }
            }
        }
    });
}
...
async function fetchAtendimentoDay({ empresaId, profSeqId, dateStr }) {
    try {
        ...
        if (atendimentoBody) {
            if (typeof atendimentoBody !== 'undefined' && atendimentoBody) { atendimentoBody.innerHTML = '<tr><td colspan="6" style="text-align:center; padding: 2rem; color: var(--text-muted);">Carregando...</td></tr>'; }
        }
        if (atendimentoEmptyState) atendimentoEmptyState.classList.add('hidden');
        ...
        if (atendimentoSummary) {
            if (typeof atendimentoSummary !== 'undefined' && atendimentoSummary) { atendimentoSummary.textContent = `${profName} — ${dateStr.split('-').reverse().join('/')} • ${itemsRows.length} itens liberados`; }
        }
        ...
        if (typeof atendimentoBody !== 'undefined' && atendimentoBody) { atendimentoBody.innerHTML = ''; }
        itemsRows.forEach(r => {
            ...
            `; }
            if (typeof atendimentoBody !== 'undefined' && atendimentoBody) { atendimentoBody.appendChild(tr); }
        });
    } catch (err) {
...
window.liberarFila = liberarFila;
window.renderAtendimentoPlaceholder = renderAtendimentoPlaceholder;
window.updateAtendimentoFinalizeButton = updateAtendimentoFinalizeButton;
window.bindAtendimentoFinalizeSingle = bindAtendimentoFinalizeSingle;
window.fetchAtendimentoForUI = fetchAtendimentoForUI;
window.fetchAtendimentoDay = fetchAtendimentoDay;
window.confirmAtendimentoItem = confirmAtendimentoItem;

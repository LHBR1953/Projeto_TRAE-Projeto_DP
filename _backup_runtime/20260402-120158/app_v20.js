    ...
    try {
        if (!contextSwitch) {
            ...
            if (!isAuth) {
                ...
                if (session === null && !shouldBlockLoginUiByBootWindow()) {
                    showLoginUi();
                } else {
                    keepCheckingUi = true;
                }
...
function showList(type = 'patients') {
    ...
    if (!bootRenderCompleted && bootPreferredTab && bootPreferredTab !== 'patients' && type === 'patients') {
        auditAuth('showList:blockedPatientsDuringBoot', { type, bootPreferredTab });
        return;
    }
...
function renderSuperAdminRenewals() {
    ...
    const rows = (activeEmpresasList || [])
        .map(e => {
            ...
            const status = String(e && e.assinatura_status || 'PENDENTE').toUpperCase();
            ...
            if (venc) {
                ...
                if (Number.isFinite(tmp.getTime())) {
                    ...
                    due = today.getTime() >= threshold.getTime();
                    diffDays = Math.ceil((vencDate.getTime() - today.getTime()) / dayMs);
                }
            }
            const isPending = status === 'PENDENTE';
            if (!isPending && !due) return null;
            return { id, nome, status, venc, vencDate, diffDays, isPending };
        })
        .filter(Boolean)
        .sort((a, b) => {
    ...
    const expired = rows.filter(r => Number.isFinite(r.diffDays) && r.diffDays < 0).length;
    const dueSoon = rows.filter(r => Number.isFinite(r.diffDays) && r.diffDays >= 0 && !r.isPending).length;
    ...
    if (kpiSub) {
        if (!rows.length) kpiSub.textContent = 'Sem pendências';
            ...
            const parts = [];
            if (pendingCount) parts.push(`${pendingCount} aguardando pagamento`);
            if (expired) parts.push(`${expired} vencida(s)`);
            if (dueSoon) parts.push(`${dueSoon} a vencer`);
            kpiSub.textContent = parts.join(' | ');
        }
    }

    if (sub) {
        if (!rows.length) sub.textContent = 'Nenhuma pendência de renovação.';
        else sub.textContent = `${rows.length} clínica(s) pendente(s) de ação`;
    }
    ...
    rows.forEach(r => {
        ...
        tr.innerHTML = `
            ...
            <td style="padding: 0.6rem; border-bottom: 1px solid var(--border-color);">${planoVal}</td>
...
db.auth.onAuthStateChange(async (event, session) => {
    auditAuth(`onAuthStateChange:${event}`, session);

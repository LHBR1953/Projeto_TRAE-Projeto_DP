...
function openProteseModal(order, forceReadonly = false) {
    ...
    if (pacienteSel && pacienteSel.options.length === 0) {
        ...
        if (typeof pacienteSel !== 'undefined' && pacienteSel) { pacienteSel.innerHTML = opts.join(''); }
    }

    if (labSel) {
        const opts = ['<option value="">Selecione...</option>'];
        (proteseLabs || []).filter(l => l.ativo !== false).forEach(l => opts.push(`<option value="${l.id}">#${l.seqid} - ${l.nome}</option>`));
        if (typeof labSel !== 'undefined' && labSel) { labSel.innerHTML = opts.join(''); }
    }
    ...
    if (protSel) {
        ...
        (professionals || [])
            ...
            .forEach(p => opts.push(`<option value="${p.id}">${p.nome}</option>`));
        if (typeof protSel !== 'undefined' && protSel) { protSel.innerHTML = opts.join(''); }
    }
    ...
    if (btnSave) {
        if (forceReadonly) {
            ...
            if (typeof btnSave !== 'undefined' && btnSave) { btnSave.style.opacity = '0.5'; }
            if (typeof btnSave !== 'undefined' && btnSave) { btnSave.style.cursor = 'not-allowed'; }
            ...
            if (typeof btnSave !== 'undefined' && btnSave) { btnSave.style.opacity = '1'; }
            if (typeof btnSave !== 'undefined' && btnSave) { btnSave.style.cursor = 'pointer'; }
            btnSave.title = '';
        }
    }
    ...
    mainInputs.forEach(id => {
        ...
        if (el) {
            if (typeof el !== 'undefined' && el) { el.disabled = forceReadonly; }
            if (forceReadonly) {
                if (typeof el !== 'undefined' && el) { el.style.opacity = '0.7'; }
            } else {
                if (typeof el !== 'undefined' && el) { el.style.opacity = '1'; }
            }
        }
    });
    ...
    if (timeline) {
        if (currentProteseOrder && currentProteseOrder.id) {
            ...
            if (typeof timeline !== 'undefined' && timeline) { timeline.innerHTML = '<div style="text-align:center; padding: 1rem; color: var(--text-muted);">Salve a OP para iniciar o histórico.</div>'; }
        }
    }
    ...
    if (anexos) {
        if (currentProteseOrder && currentProteseOrder.id) {
            ...
            if (typeof anexos !== 'undefined' && anexos) { anexos.innerHTML = '<div style="color: var(--text-muted);">Salve a OP para enviar anexos.</div>'; }
        }
    }
...
function openProteseReportsModal() {
    ...
    `;
    if (typeof body !== 'undefined' && body) { body.innerHTML = html; }
    ...
    if (!modal.dataset.bound) {
        ...
        if (typeof modal !== 'undefined' && modal) { modal.addEventListener('click', (e) => { if (e.target === modal) close(); }); }
...
function syncProteseEventButtons() {
    ...
    const setBtn = (btn, enabled) => {
        if (!btn) return;
        if (typeof btn !== 'undefined' && btn) { btn.disabled = !enabled; }
        if (typeof btn !== 'undefined' && btn) { btn.style.opacity = enabled ? '1' : '0.55'; }
    };
    ...
    if (quickSel) {
        if (typeof quickSel !== 'undefined' && quickSel) { quickSel.disabled = !enabled; }
        if (typeof quickSel !== 'undefined' && quickSel) { quickSel.style.opacity = enabled ? '1' : '0.55'; }
        if (!enabled) {
    ...
    if (btnCustodia) {
        ...
        if (typeof btnCustodia !== 'undefined' && btnCustodia) { btnCustodia.disabled = !canCustody; }
        if (typeof btnCustodia !== 'undefined' && btnCustodia) { btnCustodia.style.opacity = canCustody ? '1' : '0.55'; }
    }
}
...
function openProteseCustodiaWithDefaults({ acao, de, para, autoGenerate, context } = {}) {
    ...
    try {
        const quickSel = document.getElementById('proteseQuickEventType');
        if (quickSel && acao) if (typeof quickSel !== 'undefined' && quickSel) { quickSel.value = String(acao); }
    } catch { }
    ...
    setTimeout(() => {
        ...
        if (acaoEl && acao) {
            if (typeof acaoEl !== 'undefined' && acaoEl) { acaoEl.value = String(acao); }
            try { acaoEl.dispatchEvent(new Event('change', { bubbles: true })); } catch { }
        }
        if (deEl && de) if (typeof deEl !== 'undefined' && deEl) { deEl.value = String(de); }
        if (paraEl && para) if (typeof paraEl !== 'undefined' && paraEl) { paraEl.value = String(para); }
        if (autoGenerate) {
...
async function loadProteseAnexos(ordemId) {
    ...
    if (!list) return;
    if (!ordemId) {
        if (typeof list !== 'undefined' && list) { list.innerHTML = '<div style="color: var(--text-muted);">Salve a OP para enviar anexos.</div>'; }
        return;
    }
    try {
        ...
        if (!rows.length) {
            if (typeof list !== 'undefined' && list) { list.innerHTML = '<div style="color: var(--text-muted);">Nenhum anexo.</div>'; }
            return;
        }
        if (typeof list !== 'undefined' && list) { list.innerHTML = rows.map(r => {
        ...
        const msg = err && err.message ? err.message : 'Erro desconhecido';
        if (typeof list !== 'undefined' && list) { list.innerHTML = `<div style="color: var(--danger-color);">Erro ao carregar anexos: ${escapeHtml(msg)}</div>`; }
    }
}
...
function updateProteseItemInfo() {
    ...
    const applyExec = (v) => {
        if (execSel && v && String(execSel.value || '') !== v) if (typeof execSel !== 'undefined' && execSel) { execSel.value = v; }
        syncProteseExecucaoGroups();
    };
    ...
    if (desiredExec === 'EXTERNA') {
        ...
        if (protSel) protSel.value = '';
        if (labSel && labId) if (typeof labSel !== 'undefined' && labSel) { labSel.value = labId; }
        ...
        if (protSel && rawProt) {
            ...
            if (direct) {
                if (typeof protSel !== 'undefined' && protSel) { protSel.value = String(direct.id); }
            } else {
    ...
    if (prazoEl && !String(prazoEl.value || '').trim()) {
        if (desiredExec === 'INTERNA') {
            if (typeof prazoEl !== 'undefined' && prazoEl) { prazoEl.value = addDays(5); }
            ...
            if (typeof prazoEl !== 'undefined' && prazoEl) { prazoEl.value = addDays(dias); }
        }
    }
...
async function syncProteseUniqueOpGuard() {
    ...
    const clear = () => {
        ...
        if (typeof warnText !== 'undefined' && warnText) { warnText.textContent = ''; }
    ...
    try {
        ...
        if (typeof warnText !== 'undefined' && warnText) { warnText.textContent = `Este item já possui uma Ordem de Prótese ativa (Nº ${opNum}).`; }
...
async function loadProteseTimeline(orderId) {
    ...
    if (!wrap) return;
    if (!orderId) {
        if (typeof wrap !== 'undefined' && wrap) { wrap.innerHTML = '<div style="text-align:center; padding: 1rem; color: var(--text-muted);">Salve a OP para iniciar o histórico.</div>'; }
        return;
    }
    if (typeof wrap !== 'undefined' && wrap) { wrap.innerHTML = '<div style="text-align:center; padding: 1rem; color: var(--text-muted);">Carregando...</div>'; }
    try {
        ...
        if (!all.length) {
            if (typeof wrap !== 'undefined' && wrap) { wrap.innerHTML = '<div style="text-align:center; padding: 1rem; color: var(--text-muted);">Nenhum evento registrado.</div>'; }
            return;
        }
        ...
        if (typeof wrap !== 'undefined' && wrap) { wrap.innerHTML = warnHtml + `
        ...
        if (typeof wrap !== 'undefined' && wrap) { wrap.innerHTML = '<div style="text-align:center; padding: 1rem; color: var(--danger-color);">Falha ao carregar histórico.</div>'; }
    }
}
...
async function openProteseCustodiaModal() {
    ...
    if (typeof body !== 'undefined' && body) { body.innerHTML = `
    ...
    if (btnGerar) btnGerar.addEventListener('click', async () => {
        ...
        if (typeof btnGerar !== 'undefined' && btnGerar) { btnGerar.disabled = true; }
        if (typeof btnGerar !== 'undefined' && btnGerar) { btnGerar.style.opacity = '0.6'; }
        ...
        try {
                ...
                if (!skipSameActionConfirm && !lastErr && Array.isArray(lastRows) && lastRows.length) {
                    ...
                    if (lastA === curA) {
                        ...
                        if (!hasOppAfter) {
                            ...
                            if (!proceed) {
                                if (typeof btnGerar !== 'undefined' && btnGerar) { btnGerar.disabled = false; }
                                if (typeof btnGerar !== 'undefined' && btnGerar) { btnGerar.style.opacity = '1'; }
                                return;
                            }
                        }
                    }
            ...
            if (out) {
                if (typeof out !== 'undefined' && out) { out.style.display = ''; }
                if (typeof out !== 'undefined' && out) { out.innerHTML = `
                ...
                if (btnCopy) btnCopy.addEventListener('click', async () => {
                    ...
                    const tryExecCommand = () => {
                        ...
                        try {
                            ...
                            if (typeof ta !== 'undefined' && ta) { ta.value = link; }
                            if (typeof ta !== 'undefined' && ta) { ta.setAttribute('readonly', ''); }
                            if (typeof ta !== 'undefined' && ta) { ta.style.position = 'fixed'; }
                            if (typeof ta !== 'undefined' && ta) { ta.style.left = '-9999px'; }
                            if (typeof ta !== 'undefined' && ta) { ta.style.top = '0'; }
...
async function refreshProteseCustodiaEvents(ordemId, tryMirrorToTimeline) {
    ...
    if (!wrap) return;
    if (typeof wrap !== 'undefined' && wrap) { wrap.innerHTML = 'Carregando...'; }
    ...
    try {
        ...
        if (!rows.length) {
            if (typeof wrap !== 'undefined' && wrap) { wrap.innerHTML = '<div style="color: var(--text-muted);">Nenhuma confirmação registrada ainda.</div>'; }
            return;
        }
        ...
        if (typeof wrap !== 'undefined' && wrap) { wrap.innerHTML = html; }
        ...
        const msg = e && e.message ? e.message : 'Erro desconhecido';
        if (typeof wrap !== 'undefined' && wrap) { wrap.innerHTML = `<div style="color: var(--danger-color);">Falha ao carregar custódia: ${msg}</div>`; }
    }
}
...
function renderProteseLabsTable() {
    ...
    rows.forEach(l => {
        ...
        `; }
        if (typeof body !== 'undefined' && body) { body.appendChild(tr); }
    });
}
...
window.getInventoryAreaLabel = getInventoryAreaLabel;
window.fetchProteseFromUI = fetchProteseFromUI;
window.fetchProteseData = fetchProteseData;
window.renderProteseTable = renderProteseTable;
window.openProteseModal = openProteseModal;
window.openProteseReportsModal = openProteseReportsModal;
window.syncProteseEventButtons = syncProteseEventButtons;
window.syncProteseMaterialUi = syncProteseMaterialUi;
window.openProteseCustodiaWithDefaults = openProteseCustodiaWithDefaults;
window.loadProteseAnexos = loadProteseAnexos;
window.uploadProteseAnexo = uploadProteseAnexo;
window.ensureProteseRecebidoClinicaFromCustodia = ensureProteseRecebidoClinicaFromCustodia;
window.syncProteseExecucaoGroups = syncProteseExecucaoGroups;
window.updateProteseItemInfo = updateProteseItemInfo;
window.syncProteseUniqueOpGuard = syncProteseUniqueOpGuard;
window.openProteseById = openProteseById;
window.loadProteseTimeline = loadProteseTimeline;
window.addProteseTimelineEvent = addProteseTimelineEvent;
window.saveProteseOrder = saveProteseOrder;
window.printProteseOrder = printProteseOrder;
window.printProteseOrderSimple = printProteseOrderSimple;
window.isProteseCustodiaEnabled = isProteseCustodiaEnabled;
window.getProteseCustodiaBaseUrl = getProteseCustodiaBaseUrl;
window.openProteseCustodiaModal = openProteseCustodiaModal;
window.closeProteseCustodiaModal = closeProteseCustodiaModal;
window.refreshProteseCustodiaEvents = refreshProteseCustodiaEvents;
window.openProteseLabsModal = openProteseLabsModal;
window.renderProteseLabsTable = renderProteseLabsTable;
window.saveProteseLab = saveProteseLab;
window.ensureProteseLabsLoaded = ensureProteseLabsLoaded;
window.ensureProteseOrdersLoaded = ensureProteseOrdersLoaded;
window.openProtesePayablesModal = openProtesePayablesModal;
window.fetchProtesePayablesFromUI = fetchProtesePayablesFromUI;
window.fetchProtesePayables = fetchProtesePayables;
window.getProtesePayableDestName = getProtesePayableDestName;
window.renderProtesePayablesTable = renderProtesePayablesTable;
window.printProtesePayablesReport = printProtesePayablesReport;
